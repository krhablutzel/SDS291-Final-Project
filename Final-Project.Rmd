...
# compare public and uninsured

```{r}
# reorder levels to compare public ot uninsured
upData$Insurance <- upData$Insurance %>%
  factor(levels = c("Public", "Private", "Uninsured"))
```

```{r}
# full model w/ public first
fullModelPublic <- glm(HasUsualPlace~Age+Race+Income+Insurance, family="binomial", data = upData)
...
We fit a logistic regression model predicting HasUsualPlace from Income and Insurance when accounting for Age and Race. Our empirical logit plots with 40 bins (?) appeared linear for both HasUsualPlace vs. Income and HasUsualPlace vs. Age, so we were confident in the linearity of our data. We selected one participant per household to ensure independence, and participants were selected _____ to ensure randomness. (check survey description page 8 https://www.cdc.gov/nchs/nhis/1997-2018.htm) Additionally, we formed 2x2 contingency tables of HasUsualPlace vs. Insurance, HasUsualPlace vs. Race, and Race vs. Insurance to ensure sufficient sample sizes within each category. Our predictions may be least accurate among Alaskan Native or American Indian people, Pacific Islanders, and people of multiple races due to small sample sizes.
